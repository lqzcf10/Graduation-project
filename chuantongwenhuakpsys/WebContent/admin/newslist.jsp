...
String url="/chuantongwenhuakpsys/admin/newslist.jsp?1=1";
...
<div class="page-container">
   <form action="/chuantongwenhuakpsys/admin/newslist.jsp">
   <div class="text-c"> 
		<input type="text" class="input-text" style="width:250px" placeholder="输入标题..." id="" name="key" value="<%=key %>">
	...
	<table class="table table-border table-bordered table-bg">
		...
		for(HashMap map:list){
		 ...
		 <tr class="text-c">
				...
				<td class="td-manage">
				 <a title="编辑" href="/chuantongwenhuakpsys/admin/newsedit.jsp?id=<%=map.get("id") %>" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont"></i>编辑</a>
				 <a title="删除" href="javascript:void(0)"  class="ml-5" style="text-decoration:none" onclick="if(confirm('是否删除？')){location.replace('<%=path %><%=path %>?ac=newsDel&id=<%=map.get("id") %>');}"><i class="Hui-iconfont"></i>删除</a>
				 </td>
			</tr>
	   <%} %>
		</tbody>
...
function admin_add(title,url,w,h){
	window.location.href="/chuantongwenhuakpsys/admin/newsadd.jsp"
}
/*管理员-删除*/
