...
String url="/chuantongwenhuakpsys/admin/feiyilist.jsp?1=1";
...
%>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>诗词美图管理<span class="c-gray en">&gt;</span> 诗词美图列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
   <form action="/chuantongwenhuakpsys/admin/feiyilist.jsp">
   <div class="text-c"> 
		<input type="text" class="input-text" style="width:250px" placeholder="输入名称..." id="" name="key" value="<%=key %>">
	...
	<table class="table table-border table-bordered table-bg">
		...
		for(HashMap map:list){
		 ...
		 <tr class="text-c">
				<td><%=map.get("title") %></td>
				<td>
				<img alt="" src="/chuantongwenhuakpsys/upfile/<%=filename[0]%>" style="width:80px;height: 40px;" onclick="selectphoto('<%=map.get("id") %>')">
				</td>
				<td><%=map.get("savetime") %></td>
				
				<td class="td-manage">
				 <a title="编辑" href="/chuantongwenhuakpsys/admin/feiyiedit.jsp?id=<%=map.get("id") %>" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont"></i>编辑</a>
				 <a title="删除" href="javascript:void(0)"  class="ml-5" style="text-decoration:none" onclick="if(confirm('是否删除？')){location.replace('<%=path %><%=path %>?ac=feiyiDel&id=<%=map.get("id") %>');}"><i class="Hui-iconfont"></i>删除</a>
				 </td>
			</tr>
	   <%} %>
		</tbody>
...
function selectphoto(id){
	$.getJSON('/chuantongwenhuakpsys/chuantongwenhuakpsys?ac=jqselectPhoto&id='+id, function(msg){
		  layer.photos({
			  area: ['800px','500px'],
		    photos: {
...
function admin_add(title,url,w,h){
	window.location.href="/chuantongwenhuakpsys/admin/feiyiadd.jsp"
}
/*管理员-删除*/
