...
String hql="select * from member where  delstatus='0'";
String url="/chuantongwenhuakpsys/admin/memberlist.jsp?1=1";
if(!key.equals("")){hql+=" and (tname like '%"+key+"%' or uname like '%"+key+"%')";url+="&key="+key;}
...
<div class="page-container">
   <form action="/chuantongwenhuakpsys/admin/memberlist.jsp">
   <div class="text-c"> 
	...
	<form action="">
		...
		<tbody>
		<%
		for(HashMap member:list){
		 %>
		 <tr class="text-c">
				<td><%=member.get("uname") %></td>
				<td><%=member.get("upass") %></td>
				<td><%=member.get("tname") %></td>
				<td><%=member.get("sex") %></td>
				<td><%=member.get("tel") %></td>
				<td class="td-manage">
				 <a title="删除" href="javascript:void(0)"  class="ml-5" style="text-decoration:none" onclick="if(confirm('是否删除？')){location.replace('<%=path %><%=path %>?ac=memberDel&id=<%=member.get("id") %>');}"><i class="Hui-iconfont"></i>删除</a>
				 </td>
			</tr>
	   <%} %>
		</tbody>
