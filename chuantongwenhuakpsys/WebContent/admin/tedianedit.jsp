...
<%
	...
	HashMap map = dao.select("select * from tedian where id="+id).get(0);
...
<article class="page-container">
	<div style="width: 400px;">
	 <form class="form form-horizontal" id="form-admin-add"  action="<%=path %><%=path %>?ac=tedianedit&id=<%=id %>" method="post">
	 <input type="hidden" name="type" value=""/>
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>图片：</label>
		<div class="formControls col-xs-8 col-sm-9" >
		<img alt="" src="/chuantongwenhuakpsys/upfile/<%=map.get("filename") %>" id="imgfilename" width="100" height="100">
		<input name='filename' type="hidden" class="col-xs-10 col-sm-5"required value="<%=map.get("filename") %>"   id='url'/>&nbsp;<br>
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9" >
		<ul>
		  <li><input type="text" class="input-text"  placeholder="名称"  id="name" value="<%=map.get("name") %>" name="name"  required></li>
		  <li id="msgLi"></li>
		</ul>
		</div>
	</div>
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<textarea style="width:700px;height:300px;" name="content" id="editor_id"><%=map.get("content") %></textarea>
		</div>
	</div>
