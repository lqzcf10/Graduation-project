...
String hql="select * from lishi where 1=1 ";
String url="/chuantongwenhuakpsys/admin/lishilist.jsp?1=1";
...
%>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>名句赏析管理<span class="c-gray en">&gt;</span> 名句赏析列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
   <form action="/chuantongwenhuakpsys/admin/lishilist.jsp">
   <div class="text-c"> 
		<input type="text" class="input-text" style="width:250px" placeholder="输入作者名称..." id="" name="key" value="<%=key %>">
	...
	<table class="table table-border table-bordered table-bg">
		...
		for(HashMap map:list){
		 ...
		 <tr class="text-c">
				...
				<td class="td-manage">
				 <a title="编辑" href="/chuantongwenhuakpsys/admin/lishiedit.jsp?id=<%=map.get("id") %>" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont"></i>编辑</a>
				 <a title="删除" href="javascript:void(0)"  class="ml-5" style="text-decoration:none" onclick="if(confirm('是否删除？')){location.replace('<%=path %><%=path %>?ac=lishiDel&id=<%=map.get("id") %>');}"><i class="Hui-iconfont"></i>删除</a>
				 </td>
			</tr>
	   <%} %>
		</tbody>
...
function admin_add(title,url,w,h){
	window.location.href="/chuantongwenhuakpsys/admin/lishiadd.jsp"
}
/*管理员-删除*/
